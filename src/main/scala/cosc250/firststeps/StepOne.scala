...
object StepOne {
     ...
     def timesPosition(arr:Array[Int]):Array[Int] = {
          arr.zipWithIndex.map(tuple => tuple._1 * tuple._2)
     }
    ...
    Again this test just verifies the syntax produces the same thing
     ...
     def matchingLetters(wordA:String, wordB:String):List[(Int, Int)] = {
          ...
          val letterIndexA: Seq[(Char, Int)] = wordA.zipWithIndex
          val letterIndexB: Seq[(Char, Int)] = wordB.zipWithIndex


          //Imperative solution
          /*val listBuff: ListBuffer[(Int, Int)] = new ListBuffer()

          for((aChar, aIndex) <- letterIndexA) {
               for((bChar, bIndex) <- letterIndexB){
                    if(aChar == bChar){
                         listBuff += ((aIndex, bIndex))
                    }
               }
          }
          ...
          // level
          val allLetterIndexPairs: Seq[(Char, Int, Char, Int)] = letterIndexA.map{case (lA, iA) =>
               letterIndexB.map{case (lB, iB) =>
                    (lA, iA, lB, iB)
               }
          }.flatten

          //filtering the pairs with equal letters, later to get their indices pairs
          val letterIndicesWithEqualLetters: Seq[(Char, Int, Char, Int)] =
               allLetterIndexPairs.filter{
                    case (lA, iA, lB, iB) => lA == lB
               }

          //no just returning the indices, since the letters are the same
          letterIndicesWithEqualLetters.map{case(_, iA, _, iB) => (iA, iB)}.toList
     ...
     def roman(i:Int):String = {
          ...
          //Got the solution from repo:

          // These are ordered from biggest to smallest.
          val numerals: List[(Int, String)] = List(
               1000 -> "M",
               900 -> "CM",
               100 -> "C",
               90 -> "XC",
          ...
          while (remainder > 0) {
               ...
               //AFTER THIS STEP (reducing to lowest remainder) Must build up the rest of the numeral until reach n.
